...
Η παρούσα εργασία διεξήγαγε συστηματική αξιολόγηση κλασικών μοντέλων μηχανικής μάθησης για την ανίχνευση συναισθημάτων σε ηχητικά δεδομένα, χρησιμοποιώντας τα CREMAD και IEMOCAP dataset και τρία διαφορετικά workflows. Τα αποτελέσματα αποκαλύπτουν σημαντικές πτυχές σχετικά με την επιλογή μοντέλου, την επίδραση της μείωσης διαστάσεων και την ικανότητα γενίκευσης.

### 8.1 Απόδοση Μοντέλων ανά Workflow

Στο **80-20 workflow**, το **XGBoost** ήταν το καλύτερο μοντέλο (accuracy 62.24%, AUC 0.8542), ακολουθούμενο από το **SVM** (61.25%) και το **Logistic Regression** (59.89%). Στο **PCA workflow**, το **SVM** ανέβηκε στην πρώτη θέση (61.16%), ενώ το XGBoost υπέστη σημαντική πτώση (56.19%), αποδεικνύοντας ότι τα γραμμικά μοντέλα επωφελούνται από τη μείωση διαστάσεων, ενώ τα tree-based models χάνουν σημαντικές πληροφορίες. Στο **LOSO workflow**, το **SVM** και το **XGBoost** διατηρήθηκαν στην πρώτη θέση (~56%), με μικρή πτώση σε σχέση με το 80-20 (~6%), υποδεικνύοντας καλή γενίκευση σε άγνωστους ομιλητές.

### 8.2 Επίδραση της Μείωσης Διαστάσεων (PCA)

Η ανάλυση PCA αποκάλυψε διαφορετικές συμπεριφορές ανά κατηγορία μοντέλων:

- **Γραμμικά Μοντέλα (SVM, Logistic Regression):** Επωφελήθηκαν από το PCA, επιτυγχάνοντας παρόμοια ή ελαφρώς καλύτερες επιδόσεις. Η απομόνωση των κύριων συνιστωσών βοήθησε στην απομάκρυνση θορύβου και στην ενίσχυση των σημαντικών χαρακτηριστικών.

- **Tree-based Models (XGBoost, Random Forest, Decision Tree):** Υπέστησαν σημαντική πτώση (4-6 μονάδες accuracy). Αυτό οφείλεται στο ότι τα δέντρα αποφασιστικής λήψης βασίζονται σε non-linear διαχωρισμούς που απαιτούν την πλήρη διαστατικότητα των χαρακτηριστικών.

- **KNN:** Παρουσίασε μικρή βελτίωση (accuracy 55.65% → 55.83%), καθώς η μείωση διαστάσεων βελτίωσε την απόδοση του αλγορίθμου στον χώρο χαρακτηριστικών.

- **Naive Bayes:** Υπέστη σημαντική πτώση (accuracy 49.23% → 39.30%), υποδεικνύοντας ότι το μοντέλο εξαρτάται από συγκεκριμένες διαστάσεις που χάθηκαν κατά τη μείωση.

### 8.3 Ικανότητα Γενίκευσης (Zero-Shot Evaluation)

Η αξιολόγηση στο CREMA-D dataset αποκάλυψε ενδιαφέροντα αποτελέσματα σχετικά με την ικανότητα γενίκευσης:

- **Naive Bayes (54.29%):** Παρά το γεγονός ότι ήταν από τα χειρότερα μοντέλα στο IEMOCAP, στο CREMA-D επιδείχθηκε ως το καλύτερο. Η απλότητα του μοντέλου και η ανεξαρτησία των χαρακτηριστικών του (naive assumption) αποδείχθηκαν πλεονεκτήματα, καθώς το μοντέλο δεν υπέφερε από overfitting στις ιδιαιτερότητες του IEMOCAP dataset.

- **XGBoost (52.22%):** Διατήρησε καλή απόδοση, αλλά επηρεάστηκε από το domain shift μεταξύ IEMOCAP και CREMA-D. Η πολυπλοκότητα του μοντέλου, ενώ βοηθάει στην επίτευξη υψηλών επιδόσεων στο training dataset, μπορεί να οδηγεί σε overfitting σε συγκεκριμένες ιδιαιτερότητες του dataset.

- **SVM (50.82%):** Παρουσίασε σημαντική πτώση σε σχέση με το IEMOCAP, υποδεικνύοντας ότι τα decision boundaries που έμαθε στο IEMOCAP δεν μεταφράζονται άμεσα στο CREMA-D. Αυτό αποκαλύπτει την ευαισθησία των γραμμικών μοντέλων σε διαφορές στη διανομή των δεδομένων μεταξύ datasets.

### 8.4 Συνολική Αξιολόγηση και Συστάσεις

**Για Εφαρμογές με Παρόμοια Δεδομένα (Same-Domain):**
Το **XGBoost** είναι η καλύτερη επιλογή, επιτυγχάνοντας την υψηλότερη accuracy (62.24%) και Macro AUC (0.8542) στο 80-20 workflow. Το **SVM** αποτελεί μια εξίσου αξιόπιστη εναλλακτική, με ελαφρώς χαμηλότερη απόδοση αλλά μεγαλύτερη ερμηνευσιμότητα.

**Για Speaker-Independent Εφαρμογές:**
Το **SVM** και το **XGBoost** είναι οι καλύτερες επιλογές, με το SVM να έχει ελαφρώς καλύτερη απόδοση (56.38% vs 56.26%). Και τα δύο μοντέλα διατηρούν καλή απόδοση ακόμη και όταν δοκιμάζονται σε άγνωστους ομιλητές.

**Για Cross-Dataset Γενίκευση:**
Παράδοξα, το **Naive Bayes** αποδείχθηκε το πιο ανθεκτικό μοντέλο στο domain shift, επιτυγχάνοντας 54.29% accuracy στο CREMA-D. Αυτό υποδεικνύει ότι σε σενάρια όπου η γενίκευση σε εντελώς διαφορετικά datasets είναι κρίσιμη, απλούστερα μοντέλα μπορεί να είναι προτιμότερα.

**Σχετικά με το PCA:**
Το PCA είναι προτιμότερο όταν χρησιμοποιούνται γραμμικά μοντέλα (SVM, Logistic Regression), καθώς βελτιώνει την απόδοσή τους και μειώνει τον υπολογιστικό κόστος. Ωστόσο, δεν συνιστάται για tree-based models, καθώς οδηγεί σε σημαντική απώλεια απόδοσης.

### 8.5 Περιορισμοί και Μελλοντικές Κατευθύνσεις

**Περιορισμοί:**
1. Η απόδοση των μοντέλων (accuracy ~56-62%) είναι μέτρια για πρακτικές εφαρμογές, υποδεικνύοντας ότι τα κλασικά χαρακτηριστικά (hand-crafted features) μπορεί να μην είναι αρκετά πλούσια για την πλήρη διάκριση συναισθημάτων.

2. Το domain shift μεταξύ IEMOCAP και CREMA-D οδήγησε σε σημαντική πτώση απόδοσης, αποκαλύπτοντας την ανάγκη για domain adaptation τεχνικές.

3. Η αξιολόγηση περιορίστηκε σε 4 συναισθήματα, ενώ τα datasets περιέχουν περισσότερες κατηγορίες που θα μπορούσαν να εξερευνηθούν.

**Μελλοντικές Κατευθύνσεις:**
1. **Deep Learning Approaches:** Η χρήση deep learning models (CNN, RNN, Transformers) με raw audio ή spectrograms μπορεί να βελτιώσει σημαντικά την απόδοση, καθώς τα deep models μπορούν να μάθουν πιο πλούσια και abstract χαρακτηριστικά.

2. **Feature Engineering:** Εξερεύνηση πρόσθετων χαρακτηριστικών όπως prosody features, voice quality parameters, και temporal dynamics μπορεί να βελτιώσει την απόδοση.

3. **Domain Adaptation:** Η εφαρμογή τεχνικών domain adaptation (π.χ. Domain Adversarial Training) μπορεί να βελτιώσει την γενίκευση μεταξύ διαφορετικών datasets.

4. **Ensemble Methods:** Ο συνδυασμός πολλαπλών μοντέλων (ensemble) με διαφορετικές αρχιτεκτονικές μπορεί να οδηγήσει σε καλύτερη απόδοση και ανθεκτικότητα.

5. **Multi-Modal Approaches:** Η ενσωμάτωση πρόσθετων modalitites (π.χ. facial expressions, text transcripts) μπορεί να ενισχύσει την ακρίβεια της ανίχνευσης συναισθημάτων.
