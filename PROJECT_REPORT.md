# Αναφορά — Αναγνώριση Συναισθήματος από Ήχο

---

## 1. Σκοπός και Πλαίσιο

Το project ασχολείται με **αναγνώριση συναισθήματος (Speech Emotion Recognition)** βασισμένη σε **ακουστικά χαρακτηριστικά** και κλασικά μοντέλα machine learning. Το κύριο dataset είναι το **IEMOCAP** (Interactive Emotional Dyadic Motion Capture), ένα ευρέως χρησιμοποιούμενο corpus για έρευνα στη συναισθηματική ανάλυση ομιλίας.

**Στόχος:** Ταξινόμηση ομιλίας σε **4 συναισθηματικές κλάσεις** — *θυμωμένος (angry)*, *ευτυχισμένος (happy)*, *ουδέτερος (neutral)*, *λυπημένος (sad)* — χρησιμοποιώντας pipeline προεπεξεργασίας, εξαγωγής χαρακτηριστικών και εκπαίδευσης/αξιολόγησης πολλαπλών ταξινομητών.
...
Η εργασία οργανώνεται σε **scripts** (π.χ. `01_preprocess_data.py` → `08_one_shot_predict_eval.py`) τα οποία περιέχουν όλη τη λειτουργικότητα και χρησιμοποιούνται στα **workflows - pipelines** για την υλοποίηση τριών πειραμάτων (βλ. [Ενότητα 5](#5-δομή-pipeline-και-workflows)).
...
- **Ετικέτες:** `Angry`, `Happy`, `Neutral`, `Sad`. Οι κατηγορίες **Happy** και **Excited** του αρχικού dataset έγιναν **map** και ενώθηκαν στη μία τελική κατηγορία `Happy`. Τα υπόλοιπα labels (fea, dis, fru, sur, oth, xxx) απορρίφθηκαν.
...
Για την εξαγωγή των χαρακτηριστικών χρησιμοποιήθηκε η βιβλιοθήκη **pyAudioAnalysis** με παράθυρο 50 ms, βήμα 25 ms. Η διαδικασία ξεκινά με την κατάτμηση του σήματος σε χρονικά παράθυρα (frames), στα οποία υπολογίζονται χαρακτηριστικά τόσο στο χρονικό πεδίο (π.χ. Zero Crossing Rate, ενέργεια) όσο και στο φασματικό πεδίο μέσω μετασχηματισμού Fourier (π.χ. spectral centroid, spread, entropy, flux, MFCCs και chroma). Επιπλέον, εξάγονται τα χαρακτηριστικα για την αποτύπωση του ρυθμού μεταβολής των παραπάνω μεγεθών. Τέλος, οι τιμές των χαρακτηριστικών συνοψίζονται ανά αρχείο ομιλίας μέσω στατιστικών δεικτών (μέση τιμή, ελάχιστο, μέγιστο και τυπική απόκλιση), καταλήγοντας σε ένα τελικό διάνυσμα 272 διαστάσεων ανά δείγμα.

### 3.2 CREMA-D

Το **CREMA-D** χρησιμοποιείται στο project **μόνο για zero-shot αξιολόγηση**: τα μοντέλα εκπαιδεύονται αποκλειστικά στο IEMOCAP και στη συνέχεια αξιολογούνται στο CREMA-D, ώστε να ελεγχθεί η γενίκευση.
...
- **Μετά την προεπεξεργασία:** Απέμειναν **4.900 δείγματα** (από 7.442 τα 2.542 απορρίφθηκαν).
...
## 4. Επιλογή Μετρικών Αξιολόγησης

Στην παρούσα μελέτη, η επιλογή των μετρικών αξιολόγησης  με γνώμονα τη φύση των δεδομένων. Όπως παρατηρούμε στον πίνακα κατανομής του IEMOCAP, υπάρχει μια σημαντική **ανισορροπία στις κλάσεις (Class Imbalance)**: τα συναισθήματα `Neutral` και `Happy` κυριαρχούν (περίπου 30% το καθένα), ενώ το `Angry` και το `Sad` υποεκπροσωπούνται (~20%).

Εξαιτίας αυτής της ανισορροπίας, η χρήση της απλής **Ακρίβειας (Accuracy)** θα μπορούσε να οδηγήσει σε παραπλανητικά συμπεράσματα. Για να αντιμετωπιστεί αυτό, υιοθετήθηκαν οι **Weighted (Σταθμισμένες)** μετρικές:
*   **Weighted F1-Score:** Συνδυάζει την ακρίβεια (Precision) και την ανάκληση (Recall), λαμβάνοντας υπόψη τη βαρύτητα (πλήθος δειγμάτων) της κάθε κλάσης.
*   **Weighted Recall:** Μας δείχνει την ικανότητα του συστήματος να διακρίνει τα συναισθήματα, χωρίς να τιμωρείται άδικα στις μικρότερες κλάσεις.

---

## 5. Δομή Pipeline και Workflows

Η αρχιτεκτονική του έργου είναι modular: κάθε ροή εργασίας (workflow) αξιοποιεί τα κοινά σενάρια επεξεργασίας από τον φάκελο scripts/, ενώ τα αποτελέσματα εξάγονται σε αποκλειστικούς φάκελους (workflows/<workflow_name>/), διασφαλίζοντας την οργάνωση των δεδομένων.

### 5.1 Workflow **IEMOCAP 80-20** (`workflows/iemocap_80_20/`)

1. **Preprocess** — IEMOCAP → εξαγωγή χαρακτηριστικών → `iemocap_features.csv`.
2. **Split** — Stratified 80-20, **κανονικοποίηση (StandardScaler)** στο train και εφαρμογή στο test· αποθήκευση `scaler.pkl`.
...
7. **Zero-shot** —  Έλεγχος γενίκευσης του μοντέλου σε εξωτερικά datasets (cross-corpus). Πραγματοποιείται πρόβλεψη και εκτελείται ποσοτική αξιολόγηση των μετρικών ακρίβειας.

**Διαμερισμός 80-20:** Train 4.424, Test 1.107· η κατανομή κλάσεων διατηρείται stratified.

### 5.2 Workflow **IEMOCAP PCA** (`workflows/iemocap_pca/`)

1. Preprocess IEMOCAP.
2. Split 80-20 **χωρίς** κανονικοποίηση.
3. **PCA** μόνο στο train (threshold αθροιστικού variance 0,99)· backup των παλιών train/test σε `*_old.csv`.
...
- **5 folds** (fold_0 … fold_4)· ανά fold: train → (προαιρετικά tuning) → evaluate → zero-shot (CREMA-D).
...
*   **SVM:** Εξαιρετική απόδοση, σχεδόν ισάξια με το XGBoost. Το RBF kernel κατάφερε να διαχωρίσει τις κλάσεις στον χώρο των 272 διαστάσεων.
*   **Logistic Regression:** Έκπληξη αποτελεί η 3η θέση, αποδεικνύοντας ότι υπάρχει μια ισχυρή γραμμική συνιστώσα στα χαρακτηριστικά που μπορεί να αξιοποιηθεί.
*   **Random Forest:** Αν και ισχυρό, βρέθηκε λίγο χαμηλότερα από τα κορυφαία μοντέλα, πιθανώς λόγω υπερβολικής προσαρμογής (overfitting) σε λιγότερο σημαντικά χαρακτηριστικά.
*   **KNN:** Μετριοπαθής επίδοση. Η ευκλείδεια απόσταση σε τόσες πολλές διαστάσεις χάνει το νόημά της (curse of dimensionality), ρίχνοντας την ακρίβεια.
...
**Ανάλυση ανά Μοντέλο:**
*   **SVM & XGBoost:** Ισοβαθμούν στην κορυφή (~56%). Το SVM δείχνει μια ελαφριά υπεροχή στην Precision, ενώ το XGBoost είναι πολύ σταθερό. Η πτώση σε σχέση με το 80-20 είναι μικρή (~6%), αποδεικνύοντας ότι έχουν μάθει γενικεύσιμα χαρακτηριστικά.
*   **Random Forest:** Εδώ πλησίασε πολύ τους πρώτους. Τα ensembles (RF, XGB) είναι γενικά πιο ανθεκτικά σε αλλαγές ομιλητών.
...
## 7. Γενίκευση σε νέο περιβάλλον (Zero-Shot & CREMA-D)

Το τελικό τεστ αξιολογεί την ικανότητα των μοντέλων να λειτουργήσουν σε ένα εντελώς νέο dataset.

![Zero Shot Heatmap](workflows/iemocap_80_20/results/Metrics_graphs/Zero_shot_heatmap.png)

*   **Η έκπληξη του Naive Bayes (54.3%):** Ενώ ήταν από τους χειρότερους στο IEMOCAP, στο CREMA-D βγήκε πρώτος! Η απλότητά του λειτούργησε ως πλεονέκτημα, αποφεύγοντας το overfitting στις ιδιαιτερότητες του IEMOCAP.
...
2.  **Ρόλος του PCA:** Βοηθάει ΜΟΝΟ αν χρησιμοποιήσουμε Logistic Regression ή SVM. Καταστρέφει την απόδοση των δενδρικών μοντέλων.
3.  **Γενίκευση:** Για πραγματικές εφαρμογές (LOSO), το σύστημα είναι αξιόπιστο (~56%). Για εφαρμογές σε εντελώς νέα περιβάλλοντα (Zero-Shot), απαιτείται προσοχή, καθώς ακόμα και απλά μοντέλα (NB) μπορεί να ξεπεράσουν τα πολύπλοκα.
